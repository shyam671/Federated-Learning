...
def train_net_fedcon(net_id, net, global_net, previous_nets, train_dataloader, test_dataloader, epochs, lr, args_optimizer, mu, alpha, temperature, args,
                      round, device="cpu"):
    net = nn.DataParallel(net)
    net.cuda()
    logger.info('Training network %s' % str(net_id))
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, target) in enumerate(train_dataloader):
            ...
            for param_index, param in enumerate(net.parameters()):
                moon_prox_reg += ((mu / (2*alpha)) * torch.norm((param - global_weight_collector[param_index])) ** 2)
                #moon_prox_reg += ((mu / (2*100)) * torch.norm((param - global_weight_collector[param_index])) ** 2)

            loss = loss1 + loss2 + moon_prox_reg
    ...
    for net_id, net in nets.items():
        ...
        if args.alg == 'fedavg':
            ...
            trainacc, testacc = train_net_fedcon(net_id, net, global_model, prev_models, train_dl_local, test_dl, n_epoch, args.lr,
                                                  args.optimizer, args.mu, args.temperature, args.alpha, args, round, device=device)
...
if __name__ == '__main__':
    ...
    if args.alg == 'moon':
        ...
        for round in range(n_comm_rounds):
            ...
            print(round, train_loss, train_acc, test_acc)
